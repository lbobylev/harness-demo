...
@Component
class Lg4jReplanDecisionNode {

    private final int maxReplans;

    Lg4jReplanDecisionNode(@Value("${harness.replanning.max-replans:2}") int maxReplans) {
        this.maxReplans = Math.max(0, maxReplans);
    }

    Map<String, Object> decide(Lg4jRunState state, Budget budget) {
        if (state.terminal()) {
